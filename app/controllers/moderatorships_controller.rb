  ...
  def create
    ...
    respond_to do |format|
      if @moderatorship.save
        flash[:notice] = I18n.t('txt.moderatorship_created')
        format.html { redirect_to(@moderatorship.user) }
        format.xml  { render :xml  => @moderatorship, :status => :created, :location => @moderatorship }
      else
        flash[:error] = I18n.t('txt.moderatorship_not_created') + "#{@moderatorship.errors.full_messages.to_sentence}" unless @moderatorship.forum_id.blank?
        format.html { redirect_to(@moderatorship.user) }
